package cn.itcast.itcast.checkpoint;
...
public class CheckpointDemo02 {
    public static void main(String[] args) throws Exception {
        ...
        env.enableCheckpointing(1000);//一般配置秒级
        //设置State状态存储介质
        /*if(args.length > 0){
        ...
        //TODO 配置重启策略
        //1.默认重启策略:如果配置了Checkpoint,但没有配置重启策略,那么默认无限重启--不用
        //2.无重启策略:如有异常,直接程序失败--不用
        ...
        //上面的设置表示:如果程序有异常失败,重启3次, 每次间隔5

        //4.失败率重启策略--偶尔使用
        //env.setRestartStrategy(RestartStrategies.failureRateRestart(
        // 3, // 达到几次(10min内重启2次)
        //  Time.of(10, TimeUnit.MINUTES), //失败率测量的时间间隔
        //  Time.of(5, TimeUnit.SECONDS) // 两次连续重启的时间间隔
        //));
        //上面的设置表示:如果10分钟内job失败不到三次,自动重启, 每次间隔5s (如果10分钟内程序失败到达3次,则程序退出)
